...
const Inventory = () => {
  ...
  return (
    <main className="space-y-6">
      ...
      {/* Low & Out of Stock Wrapper */}
      <section className={`grid grid-cols-1 md:grid-cols-2 gap-6`}>

        {/* Low Stock */}
        {isLoading || data?.some(p => p.Quantity < 10 && p.Quantity > 0) && (
          <section className={`bg-white rounded-lg shadow p-6 ${data?.some(p => p.Quantity === 0) ? "col-span-1" : "col-span-2"} `}>
            ...
            <section className={`space-y-3`}>
              {isLoading ? <StockLoading /> :
                data
                  ...
                  .map(product => (
                    <section key={product.id} className="flex items-center justify-between p-3 bg-yellow-50 rounded">
                      <section>
        ...
        {/* Out of Stock */}
        {isLoading || data?.some(p => p.Quantity === 0) && (
          <section className={`bg-white rounded-lg shadow p-6 ${data?.some(p => p.Quantity < 10 && p.Quantity != 0) ? "col-span-1" : "col-span-2"} `}>
            <section className="flex items-center mb-4">
